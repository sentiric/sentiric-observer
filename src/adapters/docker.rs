...
impl DockerIngestor {
    ...
    fn process_line(&self, line: String, container_name: &str, stream_type: &str) -> LogRecord {
        // 1. Doğrudan SUTS v4.0 LogRecord olarak parse etmeyi dene (Tam uyumlu servisler)
        ...
        // 2. Genel bir JSON mı? (Yarı uyumlu servisler - Best-Effort Enrichment)
        if let Ok(json_val) = serde_json::from_str::<Value>(&line) {
            if let Some(map) = json_val.as_object() {
                let message = map.get("message").and_then(Value::as_str).unwrap_or(&line).to_string();
                ...
                return LogRecord {
                    ...
                    trace_id: None,
                    span_id: None,
                    event: "JSON_LOG_PARSED".to_string(),
                    message,
                    attributes,
                };
            }
        ...
        // 3. JSON değilse (Legacy veya Panic), Raw Log olarak paketle
                        ...
                        if !monitored.contains_key(&id) && !id.is_empty() {
                            ...
                            tokio::spawn(async move {
                                let options = LogsOptions::<String> {
                                    ...
                                    tail: "0".into(), ..Default::default()
                                };
                                let mut stream = docker_clone.logs(&id_clone, Some(options));
                                let ingestor_logic = DockerIngestor {
                                    docker: docker_clone, tx: tx_clone.clone(),
                                    node_name: node_name_clone, monitored_containers: monitored_clone.clone(),
                                };
