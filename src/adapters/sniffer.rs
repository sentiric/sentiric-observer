...
impl NetworkSniffer {
    ...
    fn parse_headers(packet: &pcap::Packet, link_type: Linktype) -> Option<Vec<u8>> {
        ...
        if data[ip_header_start + 9] != 17 { return None; } // Only UDP
        
        let payload_start = ip_header_start + ip_header_len + 8; // IP + UDP Header
    ...
    fn build_log(&self, event: &str, msg: String, attributes: HashMap<String, Value>) -> LogRecord {
        LogRecord {
            ...
            trace_id: None, span_id: None, event: event.to_string(), 
            message: msg, attributes, smart_tags: vec![],
        }
    }
}
...
impl LogIngestor for NetworkSniffer {
    async fn start(&self) -> Result<()> {
        ...
        let device_name = if self.interface == "any" { 
            ...
            match Device::list() {
                Ok(devs) => devs.into_iter().find(|d| d.name == self.interface).map(|d| d.name).unwrap_or_else(|| "any".to_string()),
                Err(e) => { warn!("Ağ kartları listelenirken hata oluştu: {}", e); "any".to_string() }
            }
        };
        ...
        let mut cap = Capture::from_device(device_name.as_str())
            ...
            .timeout(50)
            .open()
            .context("Pcap Open Error (Root/NET_ADMIN eksik)")?;
        ...
        std::thread::spawn(move || {
            ...
            loop {
                    ...
                    Ok(packet) => {
                        if let Some(payload) = Self::parse_headers(&packet, link_type) {
                            if let Some(log) = parser_logic.process_payload(&payload, packet.header.len) {
                                // [KRİTİK PERFORMANS ÇÖZÜMÜ]: blocking_send YERİNE try_send KULLANILDI
                                // Eğer kanal doluysa sistemi bloklamaz, paketi feda eder (Telekom kuralı).
                ...
                // Drop raporlaması (Saniyede bir kez logla, spami engelle)
