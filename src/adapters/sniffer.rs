...
use std::collections::{HashMap, VecDeque}; // VecDeque eklendi
...
impl NetworkSniffer {
    ...
    fn create_sip_log(&self, data: &str, len: u32) -> Option<LogRecord> {
        ...
        // Ham SIP mesajını (kısaltılmış olarak) attribute'a ekle (Deep Inspection için)
        // Ancak log boyutunu patlatmamak için sadece ilk 500 karakteri alalım.
        let preview = if data.len() > 1000 { &data[..1000] } else { data };
    ...
    fn create_rtp_log(&self, pt: u8, len: u32) -> Option<LogRecord> {
        ...
        // RTP'de Sequence Number önemlidir, onu da alalım (Byte 2 ve 3)
        // Bu, paket kaybını hesaplamak için kritiktir.
        // Packet: [V=2,P,X,CC][M,PT][SEQ_H][SEQ_L]...
        // Header zaten soyulmuştu, o yüzden burada ham pakete erişemiyoruz (parse_headers içinde data kayboldu).
        // Şimdilik sadece PT ve Len yeterli.
...
impl LogIngestor for NetworkSniffer {
    async fn start(&self) -> Result<()> {
        ...
        std::thread::spawn(move || {
            ...
            // DEDUPLICATION MEMORY (Yankı Önleyici)
            // 'any' interface'i kullanıldığında Linux Kernel paketi hem eth0, hem docker0 hem de veth üzerinde gösterebilir.
            // Bu da aynı paketin 3 kere loglanmasına neden olur.
            // Çözüm: Son görülen 50 paketin "Parmak İzini" (Fingerprint) tutmak.
            // (Timestamp + Length + Payload Start)
            let mut seen_packets: VecDeque<(u64, u32, u32)> = VecDeque::with_capacity(50);
            ...
            loop {
                ...
                match cap.next_packet() {
                    Ok(packet) => {
                        // --- 1. DEDUPLICATION (Yankı Filtresi) ---
                        let ts_sec = packet.header.ts.tv_sec as u64;
                        let len = packet.header.len;
                        
                        // Payload'ın ilk 4 byte'ını al (Unique ID gibi davranır)
                        // Bu basit hash çakışma (collision) yaratabilir ama bu bağlamda kabul edilebilir.
                        let payload_sig = if packet.data.len() > 20 {
                            ((packet.data[16] as u32) << 24) | ((packet.data[17] as u32) << 16) | 
                            ((packet.data[18] as u32) << 8) | (packet.data[19] as u32)
                        } else { 0 };

                        let fingerprint = (ts_sec, len, payload_sig);
                        
                        if seen_packets.contains(&fingerprint) {
                            // Bu paket zaten işlendi, atla.
                            continue;
                        }
                        
                        // Ring Buffer mantığı: Dolunca en eskiyi sil
                        if seen_packets.len() >= 50 { seen_packets.pop_front(); }
                        seen_packets.push_back(fingerprint);
                        ...
                        if let Some(payload) = Self::parse_headers(&packet, link_type) {
                            if let Some(log) = parser_logic.process_payload(&payload, packet.header.len) {
                                ...
                                // Blocking yerine try_send kullanarak sistemi kitlemiyoruz.
                ...
                if dropped_packets > 0 && last_drop_report.elapsed() > Duration::from_secs(1) {
                    debug!("⚠️ Sniffer Buffer Full: {} packets dropped to prevent blocking.", dropped_packets);
                    dropped_packets = 0;
