    ...
    async fn connect(&self) -> Result<ObserverServiceClient<Channel>> {
        ...
        let endpoint = match (
            ...
            (Some(cert_path), Some(key_path), Some(ca_path)) => {
                info!(
                    event = "GRPC_CLIENT_MTLS",
                    "🔒 gRPC Client: Utilizing mTLS for upstream connection."
                );
                ...
                Channel::from_shared(self.target_url.clone())?.tls_config(tls_config)?
            }
            _ => {
                error!(
                    event = "GRPC_CLIENT_MTLS_FAIL",
                    "mTLS yapılandırması eksik."
                );
...
impl LogEmitter for GrpcEmitter {
    ...
    #[allow(dead_code)]
    async fn emit(&self, _log: LogRecord) -> Result<()> {
        Ok(())
    }
