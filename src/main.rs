...
impl ObserverService for MyObserver {
    async fn ingest_log(&self, request: Request<IngestLogRequest>) -> Result<Response<IngestLogResponse>, Status> {
        ...
        // Uzak node'dan gelen loglar 🌐 ikonu ile işaretlenir
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("👁️ Sentiric Observer v0.4.0 | Node: {} starting...", node_name);
    ...
    // --- 1. FORWARDER TASK (Harvester -> Nexus) ---
                ...
                match ObserverServiceClient::connect(url.clone()).await {
                    Ok(mut client) => {
                        ...
                        while let Ok(msg) = rx.recv().await {
                            // Zaten başka node'dan (🌐) gelen logu tekrar iletme (Döngü koruması)
                            if msg.contains("🌐") { continue; }
                            
                            let req = IngestLogRequest {
                                service_name: "NODE".into(),
                                message: msg,
                                level: "INFO".into(),
                                trace_id: "".into(),
    ...
    // B. Yeni başlayan konteynerleri dinle (Docker Events)
    tokio::spawn(async move {
    ...
    tokio::spawn(async move {
        ...
        while let Some(log_result) = stream.next().await {
            if let Ok(log) = log_result {
                ...
                // Yerel loglar 🏠 ikonu ile işaretlenir
                let formatted = format!("[{}] 🏠 [{}] [{}] {}", 
                    chrono::Utc::now().format("%H:%M:%S"),
